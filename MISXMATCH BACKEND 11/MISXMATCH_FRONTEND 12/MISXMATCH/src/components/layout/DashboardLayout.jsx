...
import {
  ...
  Hospital, Building2, ShieldCheck, Sparkles, Users, Activity, ClipboardCheck, Lock, FolderCheck, Camera, UserX,
} from "lucide-react";
...
const NAV_BY_ROLE = {
  ...
  admin: [
    ...
    { to: "/admin/deactivations", label: "Deactivation Requests", icon: UserX },
...
export default function DashboardLayout() {
  const { user, logout, deactivationRequests } = useAuth();
  ...
  const pendingDeactivations = (deactivationRequests || []).filter((r) => r.status === "pending").length;
    ...
    <div className="min-h-screen bg-app text-app flex">
      ...
      <aside className={`fixed lg:sticky top-0 z-50 h-screen w-72 shrink-0 gradient-safety text-white flex flex-col transition-transform ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        ...
        <nav className="flex-1 overflow-y-auto p-3 scrollbar-thin">
          {items.map((it) => (
            ...
            >
              ...
              {it.label === "Deactivation Requests" && pendingDeactivations > 0 && (
                <span className="ml-auto text-[10px] bg-danger text-white px-1.5 rounded-full font-bold">{pendingDeactivations}</span>
              )}
            </NavLink>
          ))}
        </nav>
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3">
            {user?.profilePhotoUrl || (user?.avatar && !user.avatar.includes("dicebear")) ? (
              <img
              ...
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-gold-400 to-gold-600 text-navy-900 font-bold text-sm flex items-center justify-center shrink-0">
                {initials}
              </div>
            )}
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        <header className="h-16 border-b border-app bg-surface/80 backdrop-blur sticky top-0 z-30">
          <div className="h-full max-w-[1600px] mx-auto w-full px-4 sm:px-6 lg:px-8 flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-2 text-xs font-semibold text-muted">
              ...
              Authorized Official Session · Encryption Active · Audit Logging Enabled
            </div>
            <div className="ml-auto flex items-center gap-2">
              ...
              <Link to={`${loc.pathname.split("/")[1] ? "/" + loc.pathname.split("/")[1] : "/app"}/notifications`} className="btn btn-ghost !p-2 relative">
                ...
                {unread > 0 && <span className="absolute -top-0.5 -right-0.5 w-4 h-4 text-[10px] bg-danger text-white rounded-full flex items-center justify-center font-bold">{unread}</span>}
              </Link>
            </div>
          </div>
        </header>
        <main className="p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto w-full flex-1" id="dashboard-main" tabIndex={-1}>
